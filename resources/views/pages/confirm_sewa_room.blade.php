...
<div class="container mx-auto mt-4 px-4">
  <a href="{{ route('detail_studio_room', $room->id) }}" class="inline-flex items-center bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded">
    ...
    <div class="bg-primary text-white p-4 rounded-3xl flex flex-col md:flex-row items-center justify-between">
      ...
      <div class="text-white p-5 mt-4 md:mt-0 max-w-md w-full mr-32">
        ...
        <form action="/review" method="POST" enctype="multipart/form-data">
            @csrf
            <input type="text" name="code_order" value="{{ $code_order }}" class="w-[500px] border border-white rounded-none py-1 px-2 mb-2 text-primary" readonly />
            <input type="text" name="order_date" value="{{ $order_date }}" class="w-[500px] border border-white rounded-none py-1 px-2 mb-2 text-primary" readonly />
            <input type="text" name="order_time" value="{{ implode(', ', $order_times) }}" class="w-[500px] border border-white rounded-none py-1 px-2 mb-3 text-primary" readonly />
